...
def balanced(m):
    ...
    left_balanced = True if is_weight(left_end) else balanced(left_end)
    right_balanced = True if is_weight(right_end) else balanced(right_end)

    weight_balanced = (length(left) * total_weight(left_end) == length(right) * total_weight(right_end))
    
    return weight_balanced and left_balanced and right_balanced
...
def with_totals(m):
    ...
    #return tree(total_weight(m), [side(length(s), with_totals(end(s))) if not is_weight(s) else s for s in sides(m)])
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, attempted_password):
