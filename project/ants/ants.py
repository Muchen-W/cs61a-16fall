...
class Bee(Insect):
    ...
    def __init__(self, armor, place=None):
        ...
        self.effect_times = {'slow': 0, 'stun': 0}
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        cnt = 0
        if self.min_range:
            while cnt < self.min_range:
                if place.entrance:
                    place = place.entrance
                    cnt += 1
                else:
                    return None
        while len(place.bees) == 0:
            if place.entrance:
                place = place.entrance
                cnt += 1
            if place is hive:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 7
    implemented = True   # Change to True to view in the GUI
    queen_cnt = 0   # class attribute maintains the count of instance
    # END Problem 9
    ...
    def __init__(self):
        ...
        self.doubled_ant = []
        QueenAnt.queen_cnt += 1   # update class attribute at each instance
        self.queen_num = self.queen_cnt   # assign current value of class attribute to instance attribute
        # END Problem 9
            ...
            while place.exit:
                ...
                if place.ant:
                    ...
                    if isinstance(ant, BodyguardAnt):
                        if ant.ant is not None and ant.ant not in self.doubled_ant:
                            ant.ant.damage *= 2
                            self.doubled_ant.append(ant.ant)
                    if ant not in self.doubled_ant:
                        ant.damage *= 2
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    #return lambda colony: action if colony.time % 2 == 0 else None
    # alternative
    def slow_action(bee, colony):
        if colony.time % 2 == 0:
            return bee.default_action(colony)
        pass
    return slow_action, 'slow'
    # END Problem EC
...
def make_stun(action):
    ...
    def stun_action(bee, colony):
        pass
    return stun_action, 'stun'
    # END Problem EC


def switch_effect(name):
    if name == 'stun':
        return make_slow
    return make_stun
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    current_action, effect_name = effect(bee.action)
    bee.effect_times[effect_name] += duration

    def affected_action(colony):
        nonlocal current_action, effect_name
        current_action(bee, colony)
        for eff in bee.effect_times:
            if bee.effect_times[eff] > 0:
                bee.effect_times[eff] -= 1
        if sum(bee.effect_times.values()) == 0: # if bee.effect_times['stun'] == 0 and bee.effect_times['slow'] == 0:
            bee.action = bee.default_action  # bee.default_action(colony)
        elif bee.effect_times[effect_name] == 0:
            current_effect = switch_effect(effect_name)
            current_action, effect_name = current_effect(current_action)
