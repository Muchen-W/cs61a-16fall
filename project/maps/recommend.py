...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    restaurant_centroid_pairs = [[find_closest(restaurant_location(restaurant), centroids),
...
def find_centroid(cluster):
    ...
    return [round(mean(coordinate), 1) for coordinate in zip(*locations)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = S_xy / S_xx, y_mean - S_xy / S_xx * x_mean, S_xy**2 / (S_xx * S_yy) 
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_r_pairs = [find_predictor(user, reviewed, feature_fn) for feature_fn in feature_fns]
    return max(predictor_r_pairs, key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            ...
            user_ratings[name] = user_rating(user, name)
        else:
            name = restaurant_name(restaurant)
            user_ratings[name] = predictor(restaurant)
    return user_ratings
    # END Question 9
